...
class PlayerUI:
    ...
    def local_edit_audio_filters(self) -> list[str]:
        ...
        if has_pitch and has_speed:
            filters.append(f"rubberband=pitch={pitch:.6f}:tempo={speed:.6f}:phase=independent:pitchq=quality")
        elif has_pitch:
            filters.append(f"rubberband=pitch={pitch:.6f}:phase=independent:pitchq=quality")
        elif has_speed:
    ...
    def apply_player_adjustment(self, action: str, delta: float) -> None:
        if action == "speed":
            self.change_speed_async(float(delta))
        elif action == "pitch":
